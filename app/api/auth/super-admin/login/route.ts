...
export const POST = async(req: NextRequest) => {
    try {
      ...
      if(!isPasswordValid) {
        ...
        const rawRefresh = await issueTokens(user.id); // Generates a secure random refresh token, hashes it, and stores the hash in the database associated with the user. The raw refresh token is returned to be sent to the client for future token refresh requests.
        ...
        buildTokenCookies(res, accessToken, rawRefresh); // Sets the access token and refresh token as HTTP-only cookies in the response, with appropriate security flags and expiration times to ensure they are stored securely on the client side.
