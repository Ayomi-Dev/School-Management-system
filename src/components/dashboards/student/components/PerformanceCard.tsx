...
export function PerformanceCard({ subjects  }: PerformanceChartProps) {
  // const subjects = [
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 bg-gray-200 rounded-full h-2">
                  <div
                    ...
                    style={{ width: `${subject.scores.map(s => s.totalScore)}%` }}
                  />
                </div>
                <span className="text-sm font-semibold text-gray-700 w-12 text-right">{subject.scores.map(s => s.totalScore)}%</span>
              </div>
            </div>
          </div>
        ))}
