...
import { useClassesList, useSubjectsList } from '@/src/hooks/queries/useAdmin';
...
export function EnrolledClassesCard({ enrolledClasses = [] }: EnrolledClassesCardProps) {
  const { data: subjectsData } = useSubjectsList()
  const { data: classData } = useClassesList()
  const subjects = useMemo(() => subjectsData?.data.data || [], [subjectsData])
        ...
        <div className="space-y-3">
          {classes.map((cls: any) => (
            <div key={cls.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition">
              <div>
                <p className="font-semibold text-gray-800">{cls.level}</p>
